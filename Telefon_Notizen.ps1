...
$ComboBox1.width                 = 200
$ComboBox1.height                = 20
@('david.tassi@ict-bz.ch','horst.lang@ict-bz.ch','julia.stadelmann@ict-bz.ch','urs.nussbaumer@ict-bz.ch','nino.antonucci@ict-bz.ch','roland.spengler@ict-bz.ch','kilian.buerli@ict-bz.ch','rainer.klatt@ict-bz.ch','julian.bruegger@ict-bz.ch') | ForEach-Object {[void] $ComboBox1.Items.Add($_)}
...
$mailbtn.Add_Click(
{
$Mail = Get-Content C:\temp\telefonie\config.txt
$From = $Mail
$To = $ComboBox1.Text
$Date = Get-Date
$text = "Telefon Nummer: "+$numberbox.Text +"`r`nE-Mail: "+$mailbox.Text+"`r`nName: "+$namebox.Text+"`r`nFirma: "+$TextBox1.Text+"`r`nBetreff: "+$betreffbox.Text+"`r`nWeitere Infos: "+$moreinfobox.Text
...
try{
    Send-MailMessage -From "$From" -to "$To" -Subject "$Subject" -Body "$text" -SmtpServer "$SMTPServer" -DeliveryNotificationOption OnSuccess
